...
ssize_t vpn_ws_uwsgi_parse(vpn_ws_peer *peer, uint8_t *modifier1, uint8_t *modifier2) {
	...
	*modifier2 = peer->buf[3];
	...
	if (peer->pos < (uint64_t)(uwsgi_pktsize + 4))
		return 0;
	...
	while(uwsgi_pktsize) {
		...
		uwsgi_pktsize -= 2;
		pkt += 2;
		if (vallen > uwsgi_pktsize)
			return -1;
		char *val = (char *) pkt;
		uwsgi_pktsize -= vallen;
		pkt += vallen;

		if (vpn_ws_peer_add_var(peer, key, keylen, val, vallen))
			return -1;
	}
